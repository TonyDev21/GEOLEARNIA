...
ConfiDibu = mpDibujo.DrawingSpect(thickness=1, circle_radius=1)
...
def registro():
    ...
    pantalla1.geometry("800x500")  #Asignamos el tamaño de la ventana
        ...
        if cv2.waitKey(1) == 27:            #Cuando oprimamos "Escape" rompe el video
            ...
            if  resultados.multi_face_landmarks:
                ...
                for rostros in resultados.multi_face_landmarks:
                   #dibujamos
                    mpDibujo.draw_landmarks(frame, rostros, mpMallaFacial.FACEMESH_TESSELATION, ConfiDibu, ConfiDibu )
...
def login():
    ...
    pantalla2.geometry("800x500")   #Creamos la ventana
...
def pantalla_principal():
    ...
    pantalla = tk.Tk()
    pantalla.geometry("800x500")  #Asignamos el tamaño de la ventana 
    pantalla.title("Login Inteligencia")       #Asignamos el titulo de la pantalla
    pantalla.config(bg='#fcfcfc')     # el color
    pantalla.resizable(width=0, height=0)    # que no se pueda modificar par maximizar
    utl.centrar_ventana(pantalla,800,500) # el calculo para centrar hacia la ventana

    # -------
    logo =utl.leer_imagen("./imagenes/fg.jpg", (200, 200))  # creamos nuestro logo para traerlo al panel
        # frame_logo
    frame_logo = tk.Frame(pantalla, bd=0, width=300, relief=tk.SOLID, padx=10, pady=10,bg='#3a7ff6')
